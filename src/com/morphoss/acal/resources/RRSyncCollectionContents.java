...
public class RRSyncCollectionContents implements ResourcesRequest {
 ...
 */
			...
			for (DavNode response : responses) {
				...
				if ( cv == null ) {
					...
					cv.put(RequestProcessor.RESOURCE_NAME, responseHref);
				...
				if ( aNode.isEmpty()
					...
					if ( cv.getAsInteger(RequestProcessor.NEEDS_SYNC) == 1 ) needSyncAfterwards = true; 
	...
	private void syncWithMultiget(Map<String, ContentValues> originalData, Object[] hrefs) {
		...
		for (int hrefIndex = 0; hrefIndex < hrefs.length; hrefIndex += nPerMultiget) {
			...
			for (DavNode response : responses) {
				...
				if (Constants.LOG_DEBUG)
					Log.println(Constants.LOGD,TAG, "Multiget response needs sync="+cv.getAsString(RequestProcessor.NEEDS_SYNC)+" for "+name );
	...
	private boolean parseResponseNode(DavNode responseNode, ContentValues cv) {
		...
		if ( prop == null ) {
			...
			if ( s != null ) {
				...
				if ( s != null ) cv.put(RequestProcessor.LAST_MODIFIED, s);

				s = prop.getFirstNodeText("getcontenttype");
				if ( s != null ) cv.put(RequestProcessor.CONTENT_TYPE, s);
			}
		}
			...
			if (in == null) {
				...
				switch (status) {
					case 200: // Status O.K.
						...
						cv.put(RequestProcessor.RESOURCE_DATA, resourceData.toString() );
						for (Header hdr : requestor.getResponseHeaders()) {
							if (hdr.getName().equalsIgnoreCase("ETag")) {
								cv.put(RequestProcessor.ETAG, hdr.getValue());
								break;
							}
						}
						cv.put(RequestProcessor.NEEDS_SYNC, 0);
	...
	public boolean doScheduleNextInstance() {
